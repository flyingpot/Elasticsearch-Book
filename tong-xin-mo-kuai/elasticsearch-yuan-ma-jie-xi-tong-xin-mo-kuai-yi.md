...
这个类实现了三个方法： 1. getName只用在\_nodes/usage接口中，只要返回一个名字就好 2. routes定义了action对应的Rest请求方法和URL 3. prepareRequest定义了实际处理请求的内容，注意最后返回一个consumer，实际执行是在BaseRestHandler中：
...
本文介绍了ES的通信模块，并梳理了整个Rest请求的处理流程：从节点启动开始，Netty接受到用户发送的Rest请求，解析并包装成对象，做HTTP相关校验，根据HTTP方法和URL匹配RestAction，action处理请求并返回。 文章的第二部分会梳理Transport请求的处理，敬请期待。
